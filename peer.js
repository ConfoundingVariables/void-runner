...
export class PeerSession {
  ...
  async link(id,offer) {
    ...
    const attach=channel=>{e[channel.label]=channel;channel.onmessage=ev=>{if(ev.data.length>262144)return;try{const m=JSON.parse(ev.data);if(this.room)this.command(e,m);else this.receive(m)}catch{}};let opened=false;const ready=()=>{if(opened)return;opened=true;if(!this.room&&channel.label==='control')channel.send(JSON.stringify({type:'hello',name:this.name}));};channel.onopen=ready;if(channel.readyState==='open')ready();};
  ...
  command(e,m) {
    if(m.type==='hello'&&!e.player){try{const p=this.room.add(m.name);e.player=p.id;this.write(e.control,{type:'joined',id:p.id,code:this.code});this.write(e.control,this.room.snapshot());this.publish()}catch(err){this.write(e.control,{type:'error',message:err.message})}return}
    if(!e.player)return;
